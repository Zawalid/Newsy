...
function renderEmptyState(filter: string, query: string) {
  const emptyState: Record<
    string,
    { alt: string; heading: string; description: string }
  > = {
    "is:unread": {
  ...
  const { alt, heading, description } =
    emptyState[query.length ? "no-results" : filter];
            ...
            <div className="flex flex-wrap gap-1">
              {email.labels
                ...
                .map((label) => (
                  <span
                    key={label}
                    className="bg-accent text-foreground rounded-sm px-2 py-1 text-xs font-medium"
                  >
                    {label}
                  </span>
                ))}
