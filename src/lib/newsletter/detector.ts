...
function checkHeaders(headers?: Headers): boolean {
  if (!headers) return false;
  return (
    headers.has("list") ||
    headers.has("list-id") ||
    headers.has("list-unsubscribe") ||
    headers.has("x-newsletter") ||
    headers.has("x-loop") ||
    headers.has("x-campaign") ||
    headers.has("x-campaign-id") ||
    headers.has("x-report-abuse")
  );
}
...
export const isNewsletter = async (email: Email): Promise<boolean> => {
  ...
  const senderDomain = email.from?.address ? extractDomain(email.from.address) : null;
...
export const getNewsletterQuery = (query: string): string => {
  ...
  const queryParts = [
    ...
    keywordFilter, // Match keywords in subject
  ];
